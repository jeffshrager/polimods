...
For voter \(i\), the vote-choice score is:

```text
|ideology_i - blue_position|
- |ideology_i - red_position|
+ identity_strength * party_identity_i
+ election_shock_i
```

- Negative values favor Blue.
- Positive values favor Red.
- `election_shock_i` is drawn independently from a normal distribution with standard deviation `election-noise` in every election.

A voter’s turnout probability is:

```text
base-turnout + turnout-sensitivity * abs(choice-score)
```

The result is clipped to the interval `[0, 1]`. Stronger preferences therefore increase participation when `turnout-sensitivity` is positive.
