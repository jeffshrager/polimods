...
to run-election
  ask voters [
    ;; Policy distance and identity are internal state.  xcor and ycor are not used.
    set choice-score
...
A voter's preference for Red over Blue is:

`distance advantage for Red + identity-strength * party-identity + noise`

The voter turns out with probability `base-turnout + turnout-sensitivity * |preference|`, limited to the range 0 to 1.
